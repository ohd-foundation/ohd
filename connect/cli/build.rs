//! Build-time codegen for the OHDC Connect-RPC client.
//!
//! Mirrors `../../storage/crates/ohd-storage-server/build.rs` so the CLI's
//! generated client is wire-compatible with the storage server's generated
//! service trait. The proto root lives in the storage component
//! (`../../storage/proto/`); we resolve that relative to `CARGO_MANIFEST_DIR`
//! at build time so the path stays correct whether the CLI is built from the
//! workspace, from `cargo install --path`, or from a source archive that
