...
impl Build {
    ...
    pub fn try_build(&mut self) -> Result<Artifacts, String> {
        ...
        // OHD patch: NDK r26's clang assembler rejects modern SHA-NI / SM3 / SM4
        // SIMD mnemonics (e.g. `vsm3rnds2`) that OpenSSL 3.6's `linux-x86_64`
        // config emits via crypto/sm{3,4}/{sm3,sm4}-x86_64.S. Force `no-asm`
        // for x86_64 Android so cross-builds succeed; the emulator-only target
        // takes a small perf hit on AES/SHA in exchange.
