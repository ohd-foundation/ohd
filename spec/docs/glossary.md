...
- **OHD Relay** — the bridging service. Forwards opaque packets between OHDC consumers and storage instances that can't accept inbound connections (phones, home servers behind NAT). Sees ciphertext only.
...
| OHD Relay | `ohd-relay` | `ohd-relay` binary |
...
- **Wire path prefix** — `/ohdc/v1/...` for OHDC operations, `/relay/v1/...` for relay routing.
